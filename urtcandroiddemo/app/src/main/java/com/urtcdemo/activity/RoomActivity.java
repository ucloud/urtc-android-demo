...
//                    .collect(Collectors.toList());
        ...
        localrenderview.init(true, new int[]{R.mipmap.video_open, R.mipmap.loudspeaker, R.mipmap.video_close, R.mipmap.loudspeaker_disable}, mOnRemoteOpTrigger, new int[]{R.id.remote_video, R.id.remote_audio});
        ...
        if(mClass == UCloudRtcSdkRoomType.UCLOUD_RTC_SDK_ROOM_LARGE){
            int whiteRole = preferences.getInt(CommonUtils.WHITE_ROLE, CommonUtils.TEACHER_ROLE);
            if(whiteRole == CommonUtils.TEACHER_ROLE){
                role =  UCloudRtcSdkStreamRole.UCLOUD_RTC_SDK_STREAM_ROLE_PUB.ordinal();
            }else{
                role = UCloudRtcSdkStreamRole.UCLOUD_RTC_SDK_STREAM_ROLE_SUB.ordinal();
            }
        }
